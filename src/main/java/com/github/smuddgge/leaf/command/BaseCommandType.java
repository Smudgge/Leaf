...
public abstract class BaseCommandType implements CommandType {
    ...
    private final @NotNull List<CommandType> subCommandTypes;

    public BaseCommandType() {
    ...
    public void initialiseSubCommands(@NotNull ConfigurationSection section) {
        ...
        for (CommandType commandType : this.subCommandTypes) {
            ...
            if (!section.getKeys().contains(commandType.getIdentifier())) {
                logger.info("&7↳ &cDisabling &7subcommand (No configuration) &f" + commandType.getIdentifier());
                toRemove.add(commandType);
                continue;
            }

            // Is the command disabled?
            if (!section.getSection(commandType.getIdentifier()).getBoolean("enabled", true)) {
                logger.info("&7↳ &eDisabling &7subcommand (Configuration disabled) &f" + commandType.getIdentifier());
                toRemove.add(commandType);
                continue;
            }

            logger.info("&7↳ &aEnabling &7subcommand &f" + commandType.getIdentifier());
        }
