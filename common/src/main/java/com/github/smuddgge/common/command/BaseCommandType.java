    ...
    public void initialiseSubCommands(ConfigurationSection section) {
        ...
        for (CommandType commandType : this.subCommandTypes) {
            ...
            if (!section.getKeys().contains(commandType.getIdentifier())) {
                MessageManager.log("&7[Commands] &7↳ &eDisabling &7sub command (No configuration) : " + commandType.getIdentifier());
            ...
            if (!section.getSection(commandType.getIdentifier()).getBoolean("enabled", true)) {
                MessageManager.log("&7[Commands] &7↳ &eDisabling &7sub command (Configuration disabled) : " + commandType.getIdentifier());
                toRemove.add(commandType);
                continue;
            }

            MessageManager.log("&7[Commands] &7↳ &aEnabling &7sub command : " + commandType.getIdentifier());
        }
